...
class TestPositiv:
    ...
    def test_step1(self, make_folders, clear_folders, make_files):
        ...
        res1 = self.checkout(f"cd {data['folder_tst']}; 7z {data['archive_type']} a {data['folder_out']}/arx2", "Everything is Ok")
    ...
    def test_step2(self, clear_folders, make_files):
        ...
        res = []
        res.append(self.checkout(f"cd {data['folder_tst']}; 7z {data['archive_type']} a {data['folder_out']}/arx2", "Everything is Ok")) # новая запаковка, чтобы тесты были независимы
        res.append(self.checkout(f"cd {data['folder_out']}; 7z {data['archive_type']} e arx2.7z -o{data['folder_ext']} -y", "Everything is Ok")) # распаковка
    ...
    def test_step1_1(self):
        ...
        res_1 = self.checkout(f"cd {data['folder_out']}; 7z {data['archive_type']} a ../out/арх2 test_file.txt", "Everithing is ok")
    ...
    def test_step2_1(self):
        ...
        res_1 = self.checkout(f"cd {data['folder_out']}; 7z {data['archive_type']} e арх2.7z -o{data['folder_ext']}", "Everithing is ok")
    ...
    def test_step3(self):
        ...
        assert self.checkout(f"cd {data['folder_out']}; 7z {data['archive_type']} t арх2.7z", "Everithing is ok"), "test 3 FAIL"

    def test_step4(self):
        """проверка команды u (обновление архива)"""
        assert self.checkout(f"cd {data['folder_out']}; 7z {data['archive_type']} u арх2.7z", "Everithing is ok"), "test 4 FAIL"

    def test_step5(self):
        """проверка команды d (удаление из архива)"""
        assert self.checkout(f"cd {data['folder_out']}; 7z {data['archive_type']} d арх.7z", "Everithing is ok"), "test 5 FAIL"

    def test_step6(self, clear_folders, make_files):
        """проверка команды l (список файлов)"""
        res = []
        res.append(self.checkout(f"cd {data['folder_tst']}; 7z {data['archive_type']} a {data['folder_out']}/арх2", "Everything is Ok"))  # новая архивация для независимого теста
        for item in make_files:
            res.append(self.checkout(f"cd {data['folder_out']}; 7z {data['archive_type']} l арх2.7z {data['folder_ext']}", f"{data['file_in_ext']}"))
    ...
    def test_step7(self, clear_folders, make_files, make_subfolder):
        ...
        res = []
        res.append(self.checkout(f"cd {data['folder_tst']}; 7z {data['archive_type']} a {data['folder_out']}/арх2", "Everything is Ok"))  # новая архивация для независимого теста
        res.append(self.checkout(f"cd {data['folder_out']}; 7z {data['archive_type']} x арх.7z -o{data['folder_ext']}", "Everithing is ok"))  # распаковка
    ...
    def test_step8(self, clear_folders, make_files):
        ...
        res = []
        for item in make_files:
            res.append(self.checkout(f"cd {data['folder_tst']}; 7z {data['archive_type']} h {item}", "Everything is Ok"))
